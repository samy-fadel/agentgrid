...
| §1-F | **L'agent ne recevait aucune métadonnée de localisation sur la recherche de capacité** | `search_capacity` (outil et route personnalisée MCP) renvoyait seulement `{"candidates": [...]}`, ignorant `searched_regions`, `allowed_regions`, `allow_region_change` et `location_note`. Quand les contraintes de localisation de l'opérateur excluaient toutes les régions, l'agent recevait `{"candidates": []}` nu, sans aucun moyen de distinguer une incompatibilité matérielle d'une contrainte géographique stricte, et proposait un faux diagnostic. Même famille que le défaut §2-B : l'outil MCP n'exposait pas ce que la bibliothèque et la route HTTP savaient faire. Corrigé : les deux surfaces MCP renvoient désormais les métadonnées complètes via le résolveur partagé `resolve_search_regions`, et l'outil MCP accepte `target_region` et `allow_region_change` |
...
| `tests/test_capacity_location_reaches_the_agent.py` | 8 | l'agent reçoit les métadonnées de localisation complètes et les contrôles associés (7/8 échouent avant correction) |
...
| **Suite complète** | **311 passed** (run du 2026-09-12T07:55Z, 2 min 49 s) | |
